...
class UnifiedTrainer(TwoStageTrainer):
    ...
    def __init__(self, config: Dict):
        # Load dataset splits
        self.dataset = config['dataset']  # 'ithor' or 'robothor'
        ...
        self.val_env = AI2ThorNoMaDEnv(
            scene_names=self.splits['val'],
            image_size=tuple(config['image_size']),
            max_episode_steps=config['max_episode_steps'],
            success_distance=config['success_distance'],
            context_size=config['context_size'],
            goal_prob=config.get('eval_goal_prob', 1.0)  # Usually 1.0 for evaluation
        )
        
        self.test_env = AI2ThorNoMaDEnv(
            scene_names=self.splits['test'],
            image_size=tuple(config['image_size']),
            max_episode_steps=config['max_episode_steps'],
            success_distance=config['success_distance'],
            context_size=config['context_size'],
    ...
    def _load_splits(self, config: Dict) -> Dict[str, List[str]]:
        ...
        splits_file = config.get('splits_file', f'./config/splits/{config["dataset"]}_splits.yaml')
        ...
        if os.path.exists(splits_file):
            ...
            splits = splitter.save_splits(config['dataset'])
...
def main():
    ...
    parser.add_argument('--dataset', type=str, choices=['ithor', 'robothor'],
